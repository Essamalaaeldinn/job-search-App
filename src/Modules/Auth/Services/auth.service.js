...
import {
  HTML_TEMPLATE_confirmEmail,
  HTML_TEMPLATE_forgetPassword,
} from "../../../utils/html-template.utils.js";
...
export const confirmOtpService = async (req, res) => {
  ...
  const confirmEmail_OTP = user.OTP.filter(
    (otp) => otp.otpType === otpEnum.CONFIRM_EMAIL
  ); // array of confirmEmail OTPs objects
  if (!confirmEmail_OTP) {
...
export const signInService = async (req, res) => {
  ...
  if (user.bannedAt || user.deletedAt) {
    return res
      .status(400)
      .json({ message: "User Can not sign in, Account is freezed" });
  }
...
export const sendForgetPasswordOtpService = async (req, res) => {
  ...
  return res
    .status(200)
    .json({ message: "Forget Password OTP sent successfully" });
};
...
export const resetPasswordService = async (req, res) => {
  ...
  const forgetPassword_OTP = user.OTP.filter(
    (otp) => otp.otpType === otpEnum.FORGET_PASSWORD
  ); // array of forgetPassword OTPs objects
...
export const refreshTokenService = async (req, res) => {
  ...
  if (new Date(user.changeCredentialTime) > new Date(refreshToken.iat * 1000)) {
    return res
      .status(404)
      .json({ message: "Your credentials have changed. Please sign in again" });
  }
  const accesstoken = signToken({
